...
smoothPolys = True if arcpy.GetParameterAsText(5) == "true" else False
...
maxOID = np.max(oidToUse)

if maxOID <= 255:
    assignmentsDType = np.uint8
elif maxOID <= 65535:
    assignmentsDType = np.uint16
else:
    assignmentsDType = np.uint32

nPoints = len(oidToUse)
# Approximate number of source points for 5% progress
fivePercentPoints = int(ceil(nPoints / 20.0) + 1)
...
# and the points
latToUse = np.radians(latToUse)
lonToUse = np.radians(lonToUse)

# turn them each into 2D grids with a latitude or longitude or cos latitude at each point
# note we don't actually need to repeat / populate the new dimension which would just waste
# memory - they will be automatically "broadcasted" on demand so latsGrid*lonsGrid etc just works
latsGrid = np.expand_dims(latRads,axis=1) #.repeat(axis=1, repeats=len(lonRads))
cosLatsGrid = np.expand_dims(cosLats,axis=1)
lonsGrid = np.expand_dims(lonRads,axis=0)

# Create numpy array for cell assignments to best point and
# work through all cells to determine OID to assign to cell.
assignments = np.empty(shape=(len(rowLats),len(colLngs)), dtype=assignmentsDType)
minScoresNP = np.empty(shape=(len(rowLats),len(colLngs)), dtype=np.float32)
minScoresNP[:] = np.inf

for i in range(nPoints):
    if (i + 1) % fivePercentPoints == 0 and i != 0:
        arcpy.AddMessage("Progress: %d/%d points (%d%%), elapsed time %d seconds"
